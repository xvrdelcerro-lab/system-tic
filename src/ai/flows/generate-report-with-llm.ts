...
function getReportStyles() {
    ...
    <style>
      ...
      .kpi-section { display: flex; gap: 2rem; justify-content: center; margin: 2rem 0; }
      .kpi-card { background: #f0f4ff; border-radius: 12px; box-shadow: 0 2px 8px #e2e8f0; padding: 1.5rem 2rem; min-width: 180px; text-align: center; }
      .kpi-title { font-size: 1rem; color: #3560AD; font-weight: 600; margin-bottom: 0.5rem; }
      .kpi-value { font-size: 2rem; font-weight: bold; color: #1a202c; margin-bottom: 0.25rem; }
      .kpi-desc { font-size: 0.9rem; color: #4a5568; }
      .kpi-bar-bg { background: #e2e8f0; border-radius: 6px; height: 12px; width: 100%; margin: 0.5rem 0; }
      .kpi-bar-fill { height: 12px; border-radius: 6px; }
      .kpi-bar-fill.sales { background: #3560AD; }
      .kpi-bar-fill.satisfaction { background: #38A169; }
      .kpi-bar-fill.shipments { background: #ED8936; }
...
function generateReportHtml(title: string, summary: string, rawData: string, reportType: string) {
    ...
    try {
      ...
      if (Array.isArray(jsonData) && jsonData.length > 0) {
        ...
        // --- KPI Section for Single Product ---
        if (reportType.toLowerCase().includes('product') && jsonData.length === 1) {
          const product = jsonData[0];
          // Dummy values for KPIs (replace with real logic as needed)
          const salesCycle = product.salesCycle || 14;
          const salesAverage = product.salesAverage || 1200;
          const satisfaction = product.satisfaction || 92;
          const avgShipments = product.avgShipments || 3.5;
          kpiSection = `
            <div class="kpi-section">
            <div class="kpi-card">
              <div class="kpi-title">Sales Cycle</div>
              <div class="kpi-value">${salesCycle}d</div>
              <div class="kpi-bar-bg"><div class="kpi-bar-fill sales" style="width:${Math.min(salesCycle/30*100,100)}%"></div></div>
              <div class="kpi-desc">Days to complete sale</div>
            </div>
            <div class="kpi-card">
              <div class="kpi-title">Sales Avg</div>
              <div class="kpi-value">${salesAverage}</div>
              <div class="kpi-bar-bg"><div class="kpi-bar-fill avg" style="width:${Math.min(salesAverage/2000*100,100)}%"></div></div>
              <div class="kpi-desc">Avg units per period</div>
            </div>
            <div class="kpi-card">
              <div class="kpi-title">Satisfaction</div>
              <div class="kpi-value">${satisfaction}%</div>
              <div class="kpi-bar-bg"><div class="kpi-bar-fill satisfaction" style="width:${satisfaction}%"></div></div>
              <div class="kpi-desc">Customer satisfaction</div>
            </div>
            <div class="kpi-card">
              <div class="kpi-title">Avg Shipments</div>
              <div class="kpi-value">${avgShipments}</div>
              <div class="kpi-bar-bg"><div class="kpi-bar-fill shipments" style="width:${Math.min(avgShipments/10*100,100)}%"></div></div>
              <div class="kpi-desc">Shipments per month</div>
            </div>
            </div>
          `;
        }
