...
export default function RawMaterialsInventoryPage() {
    ...
    try {
      ...
      const result = await generateRawMaterialsReport({
        ...
        translations: {
            ...
            totalLabel: t('report.totalLabel', { count: itemsToPrint.length }),
  ...
  return (
    <ProtectedPage pageName="catalogs.rawMaterials" pageTitle="Raw Materials">
        ...
        <CardContent>
          <div className="overflow-x-auto border rounded-md">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-[120px]">{t('table.sku')}</TableHead>
                  <TableHead className="w-[200px]">{t('table.item')}</TableHead>
                  <TableHead className="w-[150px]">{t('table.vendor')}</TableHead>
                  <TableHead className="w-[100px] text-right">{t('table.price')}</TableHead>
                  <TableHead className="w-[80px] text-center">{t('table.scale')}</TableHead>
                  <TableHead className="w-[100px] text-right">{t('table.inStock')}</TableHead>
                  ...
                  allItems.map((item) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="text-center">
                        <Badge variant="outline" className="text-xs">{item.scale}</Badge>
                      </TableCell>
                      <TableCell className="text-right font-medium text-sm">{formatNumber(item.quantity)}</TableCell>
                            ...
                            render={({ field }) => (
                              <FormItem>
                                ...
                                <Select onValueChange={field.onChange} defaultValue={field.value}>
                                    ...
                                    <SelectContent>
                                      {materialTypeOptions.map(type => (<SelectItem key={type} value={type}>{tData(`MaterialTypesData.${type}`, {}, {default: type})}</SelectItem>))}
                                    </SelectContent>
                                  </Select>
                                <FormMessage />
                              </FormItem>
