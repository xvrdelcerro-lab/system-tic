...
const WasteTable = ({ wasteData, tData }: { wasteData: WasteEntry[], tData: ReturnType<typeof useTranslations> }) => {
                ...
                <TableBody>
                    {wasteData.length > 0 ? (
                        ...
                        <TableRow key={entry.id}>
                            ...
                            <TableCell>{tData(entry.phaseName)}</TableCell>
                      ...
                      <div className="space-y-2">
                          ...
                          <Select value={selectedPrintPhase} onValueChange={setSelectedPrintPhase}>
                              ...
                              <SelectContent>
                                  ...
                                  {phases.map(phase => (
                                      <SelectItem key={phase.name} value={phase.name}>{tData(phase.name)}</SelectItem>
                                  ))}
                              </SelectContent>
                          </Select>
                      </div>
          ...
          {phasesLoading ? <Loader2 className="h-8 w-8 animate-spin text-muted-foreground" /> : (
            <Tabs defaultValue={phases.length > 0 ? phases[0].name : ''}>
                <ScrollArea className="w-full whitespace-nowrap">
                  ...
                  {phases.map(phase => (
                      <TabsTrigger key={phase.name} value={phase.name} className="data-[state=inactive]:bg-muted data-[state=inactive]:text-muted-foreground rounded-lg">
                      {tData(phase.name)}
                      </TabsTrigger>
                  ))}
                  </TabsList>
                ...
                {phases.map(phase => (
                    <TabsContent key={phase.name} value={phase.name} className="mt-0">
                        <WasteTable wasteData={wasteByPhase[phase.name] || []} tData={tData} />
                    </TabsContent>
                ))}
            </Tabs>
          )}
